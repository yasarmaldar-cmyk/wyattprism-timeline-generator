  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <section className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm">
        ...
        {!preview ? (
          <p className="text-sm text-zinc-500">Click &ldquo;Preview timeline&rdquo; to see all computed dates.</p>
        ) : (
              ...
              <table className="w-full text-xs">
                <thead className="bg-zinc-100 dark:bg-zinc-800 sticky top-0">
                  <tr>
                    ...
                    <th className="text-center px-2 py-1.5 font-medium w-12">By</th>
                    <th className="text-left px-2 py-1.5 font-medium w-24">Start</th>
                    <th className="text-left px-2 py-1.5 font-medium w-24">End</th>
                ...
                <tbody>
                  {preview.tasks.map((t) => (
                    <tr key={t.id} className="border-t border-zinc-200 dark:border-zinc-800">
                      ...
                      <td className="px-2 py-1.5 text-center text-zinc-500">{respLabel(t.responsibility)}</td>
                      <td className="px-2 py-1.5 text-zinc-600 dark:text-zinc-400">{new Date(t.startDate).toLocaleDateString("en-GB")}</td>
                      <td className="px-2 py-1.5 text-zinc-600 dark:text-zinc-400">{new Date(t.endDate).toLocaleDateString("en-GB")}</td>
                      <td className="px-2 py-1.5 text-center text-zinc-500">{t.durationDays}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-xs text-zinc-500">{preview.tasks.length} tasks</p>
          </div>
        )}
      </section>
    </div>
